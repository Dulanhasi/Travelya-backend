  ...
  try {
    ...
    if (!validUserTypes.includes(userType)) {
      return res.status(400).json({
        success: false,
        message:
          "Invalid userType. Must be: traveler, service_provider, or admin",
      });
    }
    ...
    // If traveler, create traveler record with additional fields
    if (userType === "traveler") {
      ...
      await db.query(
        `INSERT INTO travelers (userId, passportNo, nicNo, nationality, dateOfBirth, emergencyContact, emergencyContactName) 
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [
          userId,
          passportNo || null,
    ...
    // If service provider, create service provider record with additional fields
    if (userType === "service_provider") {
      ...
      await db.query(
        `INSERT INTO service_providers (userId, businessName, providerType, businessRegistrationNo, address, description) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [
          userId,
...
exports.deleteAccount = async (req, res, next) => {
  try {
    ...
    const [result] = await db.query("DELETE FROM users WHERE firebaseUid = ?", [
      firebaseUid,
    ]);
